...
class Queue:
    ...
    async def run_program(self, seq_id, rule_id, dep_id):
        print(f"RULE ID ({rule_id}): {self.program_names[seq_id]} ({seq_id}) -> {dep_id}")
    ...
    async def main(self):
        ...
        while counter < len(self.dependency_keys):
            if counter in self.tracker or counter == 0:
                for routine in self.cluster_routine[counter]:
                    temp = asyncio.create_task(self.run_program(routine["seq_id"], routine["rule_id"], counter))
                    tasks.append(temp)
                counter += 1
